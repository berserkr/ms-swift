    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is None:
            ...
            if compute_loss_func is not None:
                ...
                if self.template.sequence_parallel_size > 1:
                    # outputs.loss is gathered across all SP ranks (full sequence),
                    # but num_items_in_batch only counts the local shard's tokens.
                    num_items_in_batch = num_items_in_batch * self.template.sequence_parallel_size
